...
# 데이터 수집 중복 확인
python check_duplicates.py                # data/outputs에서 중복 ID 검사
```
...
src/config.py              # 정적 설정 클래스 (환경 변수 기본값 정의)
check_duplicates.py        # 수집된 데이터의 중복 ID 검사 유틸리티
```
...
5. **PDF 다운로드**: 각 데이터 항목의 `file.portalLink` 필드에서 PDF 자동 다운로드 → `data/pdfs/{endpoint}/` 저장
